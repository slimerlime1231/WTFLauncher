...
function setupIPCListeners() {
    ...
    window.electronAPI.onJavaDownloadProgress((data) => {
        elements.progressContainer.classList.remove('hidden');
        if (data.status === 'downloading' || data.status === 'extracting') {
            const percent = Math.round(data.progress);
            elements.progressFill.style.width = `${percent}%`;
            elements.progressPercent.textContent = `${percent}%`;
            elements.progressTask.textContent = data.message || (data.status === 'extracting' ? 'Extracting Java...' : 'Downloading Java...');
        } else if (data.status === 'completed') {
             elements.progressFill.style.width = '100%';
             elements.progressPercent.textContent = '100%';
             elements.progressTask.textContent = data.message || 'Java installed!';
             setTimeout(() => {
                 elements.progressContainer.classList.add('hidden');
             }, 3000);
        } else if (data.status === 'error') {
            elements.progressContainer.classList.add('hidden');
            showToast(data.error || 'Java download failed', 'error');
        }
    });
