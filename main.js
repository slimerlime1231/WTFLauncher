...
function findJavaPath(requiredVersion) {
    if (process.platform !== 'win32') return null;
    const commonPaths = [
        `C:\\Program Files\\Java\\jdk-${requiredVersion}`,
    ...
    for (const basePath of commonPaths) {
        ...
        try {
            if (basePath.includes(`jdk-${requiredVersion}`)) {
                 const javaPath = path.join(basePath, 'bin', 'java.exe');
                 if (fs.existsSync(javaPath)) return javaPath;
            }
            const dirs = fs.readdirSync(basePath);
            for (const dir of dirs) {
                if (requiredVersion && !dir.includes(requiredVersion) && !dir.includes(`1.${requiredVersion}`)) {
                }
...
function isJavaInstalled(version) {
    return findJavaPath(version) !== null;
}
async function downloadJava(version = 21) {
    const tempDir = path.join(os.tmpdir(), 'java-download');
    const javaInstallDir = `C:\\Program Files\\Java\\jdk-${version}`;
    try {
        ...
        switch (parseInt(version)) {
            case 8:
                downloadUrl = 'https://github.com/adoptium/temurin8-binaries/releases/download/jdk8u442-b06/OpenJDK8U-jdk_x64_windows_hotspot_8u442b06.zip';
                fileName = 'java-8.zip';
                break;
            case 17:
                ...
                fileName = 'java-17.zip';
                break;
            case 21:
            default:
                downloadUrl = 'https://github.com/adoptium/temurin21-binaries/releases/download/jdk-21.0.6%2B7/OpenJDK21U-jdk_x64_windows_hotspot_21.0.6_7.zip';
                fileName = 'java-21.zip';
                break;
        }
        const filePath = path.join(tempDir, fileName);
        if (mainWindow) {
            mainWindow.webContents.send('java-download-progress', { status: 'downloading', progress: 0, message: `Starting Java ${version} download...` });
        }
        console.log(`[JAVA] Downloading Java ${version} from ${downloadUrl}`);
        ...
        response.body.on('data', (chunk) => {
            ...
            if (mainWindow) {
                mainWindow.webContents.send('java-download-progress', { 
                    status: 'downloading', 
                    progress, 
                    message: `Downloading Java ${version}: ${Math.round(downloadedSize / 1024 / 1024)}MB / ${Math.round(totalSize / 1024 / 1024)}MB` 
                });
            }
        });
        ...
        if (mainWindow) {
            mainWindow.webContents.send('java-download-progress', { status: 'extracting', progress: 50, message: `Extracting Java ${version}...` });
        }
        ...
        fs.emptyDirSync(javaInstallDir);
        fs.copySync(sourcePath, javaInstallDir);
        if (mainWindow) {
            mainWindow.webContents.send('java-download-progress', { status: 'completed', progress: 100, message: `Java ${version} installed successfully!` });
        }
        setTimeout(() => {
            try {
...
ipcMain.handle('launch-game', async (event, options) => {
    try {
        ...
        let requiredJava = 21;
        const version = options.version.number || options.version; 
        const versionMatch = version.match(/(\d+)\.(\d+)(?:\.(\d+))?/);
        if (versionMatch) {
            const minor = parseInt(versionMatch[2]);
            if (minor <= 16) requiredJava = 8;
            else if (minor <= 19) requiredJava = 17; 
            else requiredJava = 21; 
        }
        let javaObjPath = settings.javaPath;
        if (!javaObjPath || !fs.existsSync(javaObjPath)) {
             const detectedPath = findJavaPath(requiredJava);
             if (detectedPath) {
                 javaObjPath = detectedPath;
             } else {
                 const result = await dialog.showMessageBox(mainWindow, {
                    type: 'question',
                    buttons: [t('modal.save') || 'Download', t('modal.cancel') || 'Cancel'],
                    title: t('errors.javaNotFound'),
                    message: `Java ${requiredJava} ` + t('errors.javaNotFoundMessage'),
                    detail: t('errors.javaNotFoundDetail'),
                    defaultId: 0,
                    cancelId: 1
                });
                if (result.response === 0) {
                    const success = await downloadJava(requiredJava);
                    if (!success) {
                        return { success: false, error: t('errors.javaDownloadFailed') };
                    }
                    const newJavaPath = findJavaPath(requiredJava);
                    if (newJavaPath) {
                        store.set('settings.javaPath', newJavaPath);
                        javaObjPath = newJavaPath;
                    } else {
                        return { success: false, error: t('errors.javaNotFoundAfterDownload') || 'Java downloaded but not found.' };
                    }
                } else {
